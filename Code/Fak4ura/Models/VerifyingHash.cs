...
namespace Fak4ura.Models
{
    public class VerifyingHash : DbConnection
    {
        ...
        public string InsertHash(string uzytkownik_id, string hashString, string salt)
        {
            var sqlQuery = $"INSERT INTO F4_TEMPHASH (uzytkownik_id, sol , hash_string) " +
            ...
            while (!success && numberOfAttempts > 1)
            {
                try
                {
                    ...
                    cmd.Parameters.Add("val2", salt);
                    cmd.Parameters.Add("val3", hashString);
        ...
        public void GetHash(string hashString)
        {
            var success = false;
            var numberOfAttempts = 6;
            Console.WriteLine("--->@GetHash()");
            string sqlsqlQuery = $"select * from F4_temphash WHERE hash_string = :val1 " +
                $"AND data_utworzenia> sysdate -1/24";
